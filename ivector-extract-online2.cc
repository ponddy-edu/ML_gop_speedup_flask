...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    if (feature_rspecifier[0]==':') {
      char *endptr;
      int port = strtol(feature_rspecifier.c_str()+1, &endptr, 0);
      ...
      signal(SIGCHLD, SIG_IGN); //ignore return value from child
      listen(fd, 10);
      while (1) {
        ...
        if (fork() == 0) { // child
          ...
          //sprintf(buf, feature_rspecifier.c_str()+1, id);
          sprintf(buf, endptr+1, id);
          feature_rspecifier = buf;
          sprintf(buf, ivectors_wspecifier.c_str(), id, id);
          ivectors_wspecifier = buf;
          break;
        }
        close(connfd);
      }
